...
- [ ] Task: Create `conductor/tracks/linear-integration/sync-instructions.md`.
    -   Detailed prompt for an agent to perform the `Linear -> Repo` sync.
    -   Detailed prompt for an agent to perform the `Repo -> Linear` sync.
- [ ] Task: Test the sync instructions manually with a trial task.

## Phase 3: Automation Setup
Set up the automated triggers to run the sync instructions via Jules.

- [ ] Task: Define the "Monday Morning Sprint" Jules automation prompt.
- [ ] Task: Configure the periodic "Daily Sync" Jules automation.
...
- [ ] Task: Update `conductor/workflow.md` to document the new Linear integration process.
