...
class TitleScreenPresenterTest {
  ...
  @Test
  fun `initial state includes version`() = runTest {
    val navigator = FakeNavigator(TitleScreen)
    val appVersion = FakeAppVersion("1.2.3")
    val presenter = TitleScreenPresenter(navigator, appVersion)

    presenter.test {
      val initialState = awaitItem()
      assertThat(initialState.version).isEqualTo("1.2.3")
    }
  }
