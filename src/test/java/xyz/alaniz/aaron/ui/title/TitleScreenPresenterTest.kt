...
class TitleScreenPresenterTest {
  ...
  @Test
  fun `initial state contains app version`() = runTest {
    val navigator = FakeNavigator(TitleScreen)
    val presenter = TitleScreenPresenter(navigator, FakeAppVersion(version = "1.2.3"))

    presenter.test {
      val initialState = awaitItem()
      assertThat(initialState.appVersion).isEqualTo("1.2.3")
    }
  }
...
private class FakeAppVersion(override val version: String = "v1.0.0") : AppVersion
