...
4.  **Build with Gradle:** Executes `./gradlew assemble` to ensure the project compiles correctly.
5.  **Run Tests:** Executes `./gradlew test` to run all unit tests.
6.  **Upload Test Report:** If tests fail, the HTML test report is uploaded as a GitHub Action artifact named `test-report`.
